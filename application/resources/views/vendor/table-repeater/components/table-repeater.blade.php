...
<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
                ...
                <table class="w-full">
                    ...
                    <tr class="text-xs md:divide-x rtl:divide-x-reverse md:divide-gray-950/5 dark:md:divide-white/20">
                        @foreach ($headers as $key => $header)
                            <th
                            @class([
        'table-repeater-header-column px-3 py-2 font-semibold first:rounded-tl-xl rtl:first:rounded-tr-xl 
         rtl:first:rounded-tl-none last:rounded-tr-xl rtl:last:rounded-tr-none rtl:last:rounded-tl-xl',
        // Light
    ...
    style="width: {{ $header->getWidth() ?? 'auto' }}"
                                {{-- @class([
